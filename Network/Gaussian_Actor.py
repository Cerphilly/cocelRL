...
class Gaussian_Actor(nn.Module):
    ...
    def forward(self, state, deterministic=False):
        ...
        if deterministic == True:
            log_prob = dist.log_prob(mean).sum(axis=-1)
...
class Squashed_Gaussian_Actor(nn.Module):
    ...
    def dist(self, state):
        ...
        return torch.distributions.multivariate_normal.MultivariateNormal(loc=mean, covariance_matrix=torch.diag_embed(std.pow(2), offset=0, dim1=-2, dim2=-1))
...
if __name__ == '__main__':
    ...
    b = PixelEncoder((3, 84, 84), 50, layer_num=4)
    a = Squashed_Gaussian_Actor(50, 3, encoder=b)
